"""
动量策略 V4 — T+1 短线（V4全面升级版）
新增：真实RPS排名 + 市场情绪Gate + 基本面过滤 + 行业动量 + 量价相关性
"""
import pandas as pd
import numpy as np
import time
...
print("=" * 78)
print("  动量策略 V4 — T+1 今日买入→明日卖出（V4全面升级）")
print("  +真实RPS +情绪过滤 +基本面 +行业动量 +量价相关")
print("=" * 78)

# ===== Step 0: 市场情绪检测（V4新增）=====
print("\n  [0/7] 市场情绪检测...")
sentiment = get_limit_up_sentiment()
print(f"    涨停{sentiment['limit_up_count']}家  炸板率{sentiment['broken_rate']:.0f}%  市场温度{sentiment['sentiment_score']:.0f}  {sentiment['sentiment_label']}")

if sentiment["broken_rate"] > 35:
    print(f"  ❌ 炸板率{sentiment['broken_rate']:.0f}%>35%，市场承接力弱，不推荐操作")
    exit()
if sentiment["sentiment_score"] < 25:
    print(f"  ❌ 市场温度{sentiment['sentiment_score']:.0f}<25，情绪冰点，不推荐操作")
    exit()
if sentiment["limit_up_count"] < 20:
    print(f"  ⚠ 涨停仅{sentiment['limit_up_count']}家，市场偏冷，谨慎操作")
        ...
        try:
            ...
            for _, q in quotes.iterrows():
                ...
                if price <= 0 or prev <= 0:
                    continue
                chg_pct = (price / prev - 1) * 100
                is_limit_up = chg_pct >= (19.5 if (code.startswith("30") or code.startswith("68")) else 9.5)
...
# ===== Step 4: 获取换手率+资金流向+基本面（V4强化）=====
print(f"\n  [4/7] 获取换手率+资金流向+基本面...")
candidate_codes = [c["code"] for c in candidates]
extra_data = get_extra_data_batch(candidate_codes)
fund_data = get_fundamental_data_batch(candidate_codes)
print(f"    换手/资金数据: {len(extra_data)} 只  基本面数据: {len(fund_data)} 只")

# 行业动量（V4新增）
ind_momentum = get_industry_momentum()
...
for idx, cand in enumerate(candidates):
    ...
    try:
        ...
        vr_em = extra.get("volume_ratio", 0)
        ...
        # 1. 均线排列 + 斜率 (max 14) — 下调以容纳新因子
        if ind["cur"] > ind["ma5"] > ind["ma10"] > ind["ma20"]:
            if ind["ma60"] > 0 and ind["ma20"] > ind["ma60"]:
                if ind["ma5_slope"] > 0.3:
                    score += 14; details.append(("均线完美多头", 14))
                else:
                    score += 12; details.append(("均线多头(走平)", 12))
            else:
                score += 10; details.append(("短期多头排列", 10))
        elif ind["cur"] > ind["ma5"] > ind["ma10"]:
            score += 7; details.append(("站上MA5/10", 7))
        elif ind["cur"] > ind["ma5"]:
            score += 4; details.append(("站上MA5", 4))
        ...
        rsi14 = ind["rsi14"]
        if 50 <= rsi14 <= 65:
            score += 10; details.append((f"RSI{rsi14:.0f}健康", 10))
        elif 45 <= rsi14 < 50:
            score += 8; details.append((f"RSI{rsi14:.0f}中性", 8))
        elif 65 < rsi14 <= 72:
            score += 6; details.append((f"RSI{rsi14:.0f}偏高", 6))
        elif 35 <= rsi14 < 45:
            score += 4
        else:
            score += 2
        ...
        if ind["dif"] > ind["dea"] > 0 and ind["macdh"] > ind["macdh_prev"]:
            score += 10; details.append(("MACD多头加速", 10))
        ...
        # 4. 量能 (max 10)
        vr_use = max(ind["vr"], vr_em) if vr_em > 0 else ind["vr"]
        if 1.1 <= vr_use <= 2.5:
        ...
        dev = ind["dev"]
        if 2 <= dev <= 12:
            score += 5; details.append((f"乖离{dev:.1f}%适中", 5))
        elif -2 <= dev < 2:
            score += 3
        elif 12 < dev <= 18:
            score += 2
        ...
        if div_type is None and vr_use >= 1.0:
            score += 10; details.append(("量价配合良好", 10))
        elif div_type is None:
            score += 6; details.append(("量价正常", 6))
        ...
        if main_net > 10000000:
            score += 8; details.append(("主力大幅流入", 8))
        elif main_net > 5000000:
            score += 6; details.append(("主力流入", 6))
        elif main_net > 0:
            score += 4; details.append(("主力微流入", 4))
        elif main_net > -5000000:
            score += 2

        # 10. 真实RPS (max 8) — V4核心升级
        if rps20 >= 85:
            score += 8; details.append((f"RPS{rps20:.0f}极强", 8))
        elif rps20 >= 70:
            score += 6; details.append((f"RPS{rps20:.0f}强势", 6))
        elif rps20 >= 55:
            score += 4; details.append((f"RPS{rps20:.0f}偏强", 4))
        elif rps20 >= 40:
            score += 2
        else:
            score += 1
        ...
        if lu >= 3:
            score += 4; details.append((f"涨停{lu}次", 4))
        elif lu >= 1:
            score += 2

        # 12. 行业动量 (max 5) — V4新增
        # 简化：行业动量分数基于行情热度
        if ind_momentum and ind_momentum.get("top_industries"):
            score += 3  # 行业有热度基础分

        # 13. 基本面质量 (max 4) — V4新增
        if 5 < pe < 50 and 1 < pb < 5:
            score += 4; details.append(("估值合理", 4))
        elif 0 < pe < 100:
            score += 2
        ...
        # 主力大幅流出
        if main_net < -10000000:
            penalty += 8; details.append(("主力大幅流出", -8))

        # 低RPS
        if rps20 < 30:
            penalty += 6; details.append((f"RPS{rps20:.0f}弱势", -6))
        ...
        results.append({
            ...
            "rps20": rps20, "rps60": rps60, "limit_up_count": lu,
...
print(f"\n  [7/7] 行业动量 TOP10:")
if ind_momentum and ind_momentum.get("top_industries"):
    for ind_name in ind_momentum["top_industries"][:10]:
        info = ind_momentum["industry_list"].get(ind_name, {})
        print(f"    {ind_name}: +{info.get('chg', 0):.1f}% (排名{info.get('rank', '?')}/{ind_momentum['total_industries']})")

# ===== 输出 TOP15 =====
print(f"\n{'=' * 78}")
print(f"  动量策略 V4 TOP15  (大盘: {env_label}  情绪: {sentiment['sentiment_label']})")
print(f"{'=' * 78}")
...
for i, r in enumerate(results[:15]):
    ...
    pe_str = f"PE={r.get('pe', 0):.1f}" if r.get("pe", 0) > 0 else "PE=亏损"
    print(f"\n  #{i + 1} {name_str}({r['code']})  {r['price']:.2f}  今日{r['chg_today']:+.1f}%  评分{r['score']}  {tag}")
    print(f"  [{bar}]")
    print(f"  RSI14={r['rsi14']:.0f}  量比={r['vr']:.1f}x  换手={r['turnover']:.1f}%  大单={r['main_net'] / 10000:.0f}万")
    print(f"  RPS20={r['rps20']:.0f}  乖离={r['dev']:.1f}%  涨停基因={r['limit_up_count']}次  {pe_str}")
    print(f"  MACD DIF={r['dif']:.3f} DEA={r['dea']:.3f}  KDJ-J={r['j']:.0f}  BOLL={r['boll_pos']:.0f}%")
    print(f"  MA5={r['ma5']:.2f} MA10={r['ma10']:.2f} MA20={r['ma20']:.2f}  风险: {warn_str}")
...
if results:
    ...
    if best.get("rps20", 50) < 35: risk_flags.append("RPS弱势")
    ...
    print(f"  大盘: {env_label}    市场情绪: {sentiment['sentiment_label']}({sentiment['sentiment_score']:.0f})")
    print(f"  换手率: {best['turnover']:.1f}%    大单净量: {best['main_net'] / 10000:.0f}万")
