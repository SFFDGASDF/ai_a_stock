"""
超跌反弹策略 V4 — 今天大跌 明天反弹（V4全面升级版）
新增：真实RPS + 情绪Gate + 基本面质量过滤 + 低位放量检测 + 行业动量
"""
import pandas as pd
import numpy as np
import time
...
print("=" * 74)
print("  超跌反弹策略 V4 — 今天大跌  明天反弹（V4全面升级）")
print("=" * 74)

# ===== Step 0: 市场情绪（V4新增）=====
print("\n  [0/7] 市场情绪检测...")
sentiment = get_limit_up_sentiment()
print(f"    涨停{sentiment['limit_up_count']}家  跌停{sentiment.get('limit_down_count', 0)}家  炸板率{sentiment['broken_rate']:.0f}%  温度{sentiment['sentiment_score']:.0f}")

# 跌停过多=恐慌，不抄底
limit_down = sentiment.get("limit_down_count", 0)
if limit_down > 50:
    print(f"  ❌ 跌停{limit_down}家(>50)，市场恐慌，超跌策略风险极高")
    exit()
...
print(f"    今日跌2-8%+>3000万: {len(candidates)} 只")
...
print(f"    换手/资金: {len(extra_data)} 只  基本面: {len(fund_data)} 只")
...
for idx, c in enumerate(candidates):
    ...
    try:
        ...
        # 10. 低位放量抄底 (max 10) — V4强化
        if div_type == "bear_divergence" and ls > 1.5:
            score += 10; details.append(("低位放量抄底", 10))
        ...
        # === V4新增惩罚 ===
        # RPS极弱
        if rps20 < 20: penalty += 8; details.append((f"RPS{rps20:.0f}极弱", -8))
...
# ===== 输出 TOP15 =====
print(f"\n{'=' * 74}")
print(f"  超跌反弹 V4 TOP15  (大盘: {env_label}  情绪: {sentiment['sentiment_label']})")
print(f"{'=' * 74}")
...
for i, r in enumerate(results[:15]):
    ...
    pe_str = f"PE={r.get('pe', 0):.1f}" if r.get("pe", 0) > 0 else "PE亏损"
    ...
    print(f"  下影={r['lower_shadow']:.1f}%  反弹潜力={r['bounce_potential']:.1f}  RPS20={r['rps20']:.0f}  {pe_str}")
    print(f"  BOLL={r['boll_pos']:.0f}%  支撑={r['support']}({r['support_dist']:.1f}%)  连阴={r['down_days']}天  风险: {warn_str}")
...
if results:
    ...
    if best.get("rps20", 50) < 25: risk_flags.append("RPS极弱")
    ...
    print(f"  大盘: {env_label}    市场情绪: {sentiment['sentiment_label']}")
    print(f"  换手率: {best['turnover']:.1f}%    大单净量: {best['main_net'] / 10000:.0f}万")
    print(f"  RPS20: {best['rps20']:.0f}    PE: {best.get('pe', 0):.1f}    反弹潜力: {best['bounce_potential']:.1f}")

    if market_score <= -2:
        print(f"\n  !! 大盘环境极弱，超跌策略风险极高，不建议买入")
    elif limit_down > 30:
        print(f"\n  !! 跌停{limit_down}家，恐慌未释放完，不抄底")
    elif risk_flags:
