...
| **交互式K线图** | ECharts 渲染，分时/日K/周K/月K切换 + MA/MACD/RSI/KDJ 多指标 + 滚轮缩放拖拽 |
...
点击任意股票行弹出 ECharts 交互式K线图：
- **四周期切换**：分时 / 日K / 周K / 月K（周月K由日线自动重采样）
- **技术指标**：MA5/MA10/MA20/MA60 + MACD(DIF/DEA/柱) + RSI14(含30/70线) + KDJ
- **交互操作**：鼠标滚轮缩放、拖拽平移、十字光标、悬停查看 OHLC 详情
- **深色主题**：与仪表盘 UI 风格统一
- **数据接口**：`GET /api/kline/<code>?period=day|week|month|minute`
...
### V4.5 — 稳定性增强 + AI分析修复 + UI优化

- **修复** AI分析全部接口404 — `dashboard.py` 缺少 `app.register_blueprint(ai_bp)` 导致 `/api/ai-analyze`、`/api/ai-history`、`/api/ai-latest-by-codes` 不可用
- **修复** 东财API间歇性 `RemoteDisconnected` — 新增模块级 `_get_em_session()` + `_em_fetch()` 统一请求通道（Session复用 + 5次指数退避重试）
- **修复** 市场情绪跌停/炸板数据永远为0 — 涨停/跌停各自独立 API 调用（`po=1` vs `po=0`），温度公式改为涨停(70%)+跌停(30%)双维动态计算
- **修复** 策略 Step [4/7] 换手率/资金/基本面全部返回0 — `_fetch_market_extra_data`、`_fetch_market_fundamental` 统一改用带重试的 `_em_fetch`
- **修复** 综合选股出现重复股票 — 后端 `code` strip清洗 + 前端 `showResult()` 二次去重兜底
- **优化** AI评级悬浮 Tooltip — 宽度从240px扩大到300px，`position:fixed` + JS动态定位防裁切，结构化为评级结论/目标价/交易结论/投资论点四层信息
- **优化** AI评级 Tooltip 内容 — `db_manager.get_ai_latest_by_codes()` 额外提取 `price_target`/`time_horizon`/`thesis` 字段
